...
分块索引文件用于优化大文件的随机访问性能，将数据分成多个块进行管理：
...
│ offset_2        │ 8          │ uint64   │ 第二个字符串在.pool文件中的偏移   │
│ ...             │ ...        │ ...      │ 更多偏移量...                     │
│ offset_n        │ 8          │ uint64   │ 第n个字符串在.pool文件中的偏移   │
└─────────────────────────────────────────────────────────────────────────────┘
```

**详细说明：**
- **string_count**: 字符串池中唯一字符串的总数量
- **offset_i**: 第i个字符串在.pool文件中的字节偏移量
- **快速查找**: 通过字符串ID直接计算偏移量位置，实现O(1)时间复杂度的字符串查找
...
│ 优势            │ 说明                                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│ 内存效率        │ 只加载需要的分块到内存，减少内存占用                      │
│ 快速定位        │ 通过二分查找快速定位要素所在的分块                        │
│ 并行访问        │ 不同分块可以并行读取，提高I/O效率                         │
│ 缓存友好        │ 分块级别的缓存管理，提高重复访问性能                      │
│ 大文件支持      │ 支持处理GB级别的数据文件                                  │
│ 按需加载        │ 支持按需加载分块，避免一次性加载所有索引                  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│ 场景            │ 使用索引类型        │ 性能特点                            │
├─────────────────────────────────────────────────────────────────────────────┤
│ 小文件 (<1MB)   │ 无索引              │ 顺序扫描，内存占用小                │
│ 中等文件 (1-100MB) │ 分块索引          │ 快速随机访问，内存可控              │
│ 大文件 (>100MB) │ 分块索引 + 缓存     │ 高效随机访问，支持并行处理          │
│ 字符串查询      │ 字符串池索引        │ O(1)时间复杂度的字符串查找          │
│ 批量处理        │ 分块索引            │ 分块并行处理，提高吞吐量            │
└─────────────────────────────────────────────────────────────────────────────┘
```

**索引文件选择策略：**
- **自动选择**: 系统根据文件大小自动选择是否使用分块索引
- **手动控制**: 可以通过配置参数控制分块大小和缓存策略
- **性能优化**: 大文件自动启用分块索引，小文件使用顺序访问
