...
class GeometrySerializer:
    ...
    @staticmethod
    def deserialize_coordinates(data: bytes) -> Tuple[List[Tuple[float, float]], int]:
        """从二进制数据反序列化坐标"""
    ...
    def deserialize_geometry(data: bytes) -> Tuple[GeometryData, int]:
        ...
        if (
            len(data) < 48
        ):  # 最小长度：8+1+8+8+8+8+8=41字节，但实际需要48字节（内存对齐）
            raise ValueError("数据长度不足，无法反序列化几何对象")

        # 解析基本字段
        feature_id, geometry_type, min_x, min_y, max_x, max_y = struct.unpack(
            "Qbdddd", data[:48]
        )
