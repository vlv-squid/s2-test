...
class PerformanceComparison:
    ...
    def __init__(self, shapefile_path: str, output_dir: str = "./output_data"):
        ...
        self.geometry_storage = GeometryStorage(f"{output_dir}/{self.shapefile_name}_geom.dat")
        self.attribute_storage = AttributeStorage(f"{output_dir}/{self.shapefile_name}_attr.dat")
        
        # 初始化S2索引
        self.s2_index = S2SpatialIndex(shapefile_path, f"./index_py/s2.pkl", resolution=15)
        
        # 测试用的查询范围
        self.test_bboxes = [
            (103.2504, 26.4297, 103.3028, 26.4747)
        ]
    ...
    def test_custom_spatial_query(self, bbox: Tuple[float, float, float, float]) -> Dict:
        """测试自定义格式空间查询性能"""
        ...
        for fid in candidate_fids:
            try:
                ...
                if not (geom_bbox[2] < bbox[0] or geom_bbox[0] > bbox[2] or 
                       geom_bbox[3] < bbox[1] or geom_bbox[1] > bbox[3]):
                    results.append(fid)
            except:
                pass
    ...
    def run_comprehensive_test(self, test_sizes: List[int] = [100, 1000, 10000]):
        ...
        for size in test_sizes:
            ...
            if ogr_result:
                print(f"OGR - 几何: {ogr_result['geom_rate']:.1f} 要素/秒, "
            ...
            if custom_result:
                print(f"自定义 - 几何: {custom_result['geom_rate']:.1f} 要素/秒, "
