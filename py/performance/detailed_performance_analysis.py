...
class DetailedPerformanceAnalysis:
    ...
    def test_sequential_read_performance(self) -> Dict:
        ...
        for size in self.test_sizes:
            ...
            if ogr_stats['total_rate'] and custom_stats['total_rate']:
                improvement = (custom_stats['total_rate']['mean'] / ogr_stats['total_rate']['mean'] - 1) * 100
    ...
    def test_spatial_query_performance(self) -> Dict:
        ...
        for i, bbox in enumerate(self.comparison.test_bboxes):
            ...
            custom_stats = {
                ...
                'result_count': self.calculate_statistics(custom_results, 'result_count'),
                'candidate_count': self.calculate_statistics(custom_results, 'candidate_count')
            }
            ...
            if ogr_stats['query_time'] and custom_stats['query_time']:
                improvement = (ogr_stats['query_time']['mean'] / custom_stats['query_time']['mean'] - 1) * 100
    ...
    def generate_performance_charts(self, sequential_results: Dict, random_results: Dict, spatial_results: Dict):
        """生成性能对比图表"""
        print("\n" + "=" * 50)
        print("生成性能对比图表")
        print("=" * 50)
        ...
        custom_rates = [sequential_results['custom'][s]['total_rate']['mean'] for s in sizes]
        ...
        custom_rates = [random_results['custom'][s]['total_rate']['mean'] for s in sizes]
        ...
        # 性能提升对比
        ax3 = axes[1, 0]
        sequential_improvements = [sequential_results['comparison'].get(s, 0) for s in sizes]
        random_improvements = [random_results['comparison'].get(s, 0) for s in sizes]
        
        ax3.plot(sizes, sequential_improvements, 'o-', label='顺序读取', linewidth=2, markersize=8)
        ax3.plot(sizes, random_improvements, 's-', label='随机读取', linewidth=2, markersize=8)
        ax3.set_xlabel('要素数量')
        ax3.set_ylabel('性能提升 (%)')
        ...
        ogr_times = [spatial_results['ogr'][b]['query_time']['mean'] * 1000 for b in bbox_names]
        custom_times = [spatial_results['custom'][b]['query_time']['mean'] * 1000 for b in bbox_names]
        ...
        # 几何数据读取性能
        ax1 = axes2[0, 0]
        ogr_geom_rates = [sequential_results['ogr'][s]['geom_rate']['mean'] for s in sizes]
        custom_geom_rates = [sequential_results['custom'][s]['geom_rate']['mean'] for s in sizes]
        ...
        # 属性数据读取性能
        ax2 = axes2[0, 1]
        ogr_attr_rates = [sequential_results['ogr'][s]['attr_rate']['mean'] for s in sizes]
        custom_attr_rates = [sequential_results['custom'][s]['attr_rate']['mean'] for s in sizes]
        ...
        # 查询结果数量对比
        ax3 = axes2[1, 0]
        ogr_counts = [spatial_results['ogr'][b]['result_count']['mean'] for b in bbox_names]
        custom_counts = [spatial_results['custom'][b]['result_count']['mean'] for b in bbox_names]
        ...
        # 查询效率对比
        ax4 = axes2[1, 1]
        ogr_efficiency = [ogr_counts[i] / (ogr_times[i] / 1000) if ogr_times[i] > 0 else 0 for i in range(len(ogr_counts))]
        custom_efficiency = [custom_counts[i] / (custom_times[i] / 1000) if custom_times[i] > 0 else 0 for i in range(len(custom_counts))]
        
        ax4.bar(x_bbox - width/2, ogr_efficiency, width, label='OGR', alpha=0.8)
        ax4.bar(x_bbox + width/2, custom_efficiency, width, label='自定义格式', alpha=0.8)
        ax4.set_xlabel('查询范围')
    ...
    def save_results(self, sequential_results: Dict, random_results: Dict, spatial_results: Dict):
        """保存测试结果到JSON文件"""
        all_results = {
            'test_config': {
            ...
            'summary': self.generate_summary(sequential_results, random_results, spatial_results)
        }
    ...
    def generate_summary(self, sequential_results: Dict, random_results: Dict, spatial_results: Dict) -> Dict:
        """生成测试总结"""
        summary = {
            'overall_performance': {},
            'key_findings': [],
        ...
        if sequential_improvements:
            ...
            if avg_seq_improvement > 0:
                summary['key_findings'].append(f"顺序读取性能平均提升 {avg_seq_improvement:.1f}%")
            else:
                summary['key_findings'].append(f"顺序读取性能平均下降 {abs(avg_seq_improvement):.1f}%")
        
        if random_improvements:
            avg_rand_improvement = statistics.mean(random_improvements)
            if avg_rand_improvement > 0:
                summary['key_findings'].append(f"随机读取性能平均提升 {avg_rand_improvement:.1f}%")
            else:
                summary['key_findings'].append(f"随机读取性能平均下降 {abs(avg_rand_improvement):.1f}%")
        
        if spatial_improvements:
            avg_spatial_improvement = statistics.mean(spatial_improvements)
            if avg_spatial_improvement > 0:
                summary['key_findings'].append(f"空间查询性能平均提升 {avg_spatial_improvement:.1f}%")
            else:
                summary['key_findings'].append(f"空间查询性能平均下降 {abs(avg_spatial_improvement):.1f}%")
        
        # 建议
        if sequential_improvements and statistics.mean(sequential_improvements) > 50:
            summary['recommendations'].append("自定义格式在顺序读取场景下表现优异，适合批量数据处理")
        
        if random_improvements and statistics.mean(random_improvements) > 50:
            summary['recommendations'].append("自定义格式在随机读取场景下表现优异，适合随机访问场景")
        
        if spatial_improvements and statistics.mean(spatial_improvements) > 50:
            summary['recommendations'].append("自定义格式在空间查询场景下表现优异，适合空间分析应用")
    ...
    def run_complete_analysis(self):
        ...
        # 4. 生成图表
        self.generate_performance_charts(sequential_results, random_results, spatial_results)
    ...
    def print_summary(self, sequential_results: Dict, random_results: Dict, spatial_results: Dict):
        """打印测试总结"""
        print("\n" + "=" * 50)
        print("性能测试总结")
        print("=" * 50)
