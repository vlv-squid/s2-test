...
#include "gisstorage/optimized_attribute_serializer.h"
...
namespace GisStorage {
    ...
    OptimizedAttributeSerializer::OptimizedAttributeSerializer() {
    ...
    std::vector<uint8_t> OptimizedAttributeSerializer::serializeAttributes(const AttributeData& attribute) {
    ...
    std::unique_ptr<AttributeData> OptimizedAttributeSerializer::deserializeAttributes(const std::vector<uint8_t>& data) {
    ...
    std::vector<uint8_t> OptimizedAttributeSerializer::serializeStringPool() const {
        return string_pool_.serialize();
    }

    void OptimizedAttributeSerializer::deserializeStringPool(const std::vector<uint8_t>& data) {
        string_pool_.deserialize(data);
    }

    OptimizedAttributeSerializer::CompressionStats OptimizedAttributeSerializer::getCompressionStats() const {
        stats_.unique_strings = string_pool_.getPoolSize();
        stats_.total_strings = string_pool_.getTotalSize();
        return stats_;
    }

    void OptimizedAttributeSerializer::updateStats(size_t original_size, size_t compressed_size) {
