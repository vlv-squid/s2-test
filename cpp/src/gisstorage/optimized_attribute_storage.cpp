...
namespace GisStorage {
    ...
    // OptimizedAttributeStorage 实现
    OptimizedAttributeStorage::OptimizedAttributeStorage(const std::string& attribute_file, const std::string& string_pool_file)
    ...
    int64_t OptimizedAttributeStorage::writeAttribute(const AttributeData& attribute) {
        // 使用追加模式
    ...
    std::unique_ptr<AttributeData> OptimizedAttributeStorage::readAttribute(uint64_t feature_id) {
    ...
    std::vector<uint64_t> OptimizedAttributeStorage::getAllFeatureIds() {
        const auto& offsets = getOffsetIndex();
    ...
    bool OptimizedAttributeStorage::hasFeature(uint64_t feature_id) {
        const auto& offsets = getOffsetIndex();
        return offsets.find(feature_id) != offsets.end();
    }

    void OptimizedAttributeStorage::clearCache() {
        offset_index_.clear();
        index_built_ = false;
    }

    void OptimizedAttributeStorage::loadIndexFromFile(const std::string& index_file) {
    ...
    void OptimizedAttributeStorage::saveStringPool() {
    ...
    void OptimizedAttributeStorage::loadStringPool() {
        if (!std::filesystem::exists(string_pool_file_)) {
    ...
    OptimizedAttributeSerializer::CompressionStats OptimizedAttributeStorage::getCompressionStats() const {
        return serializer_.getCompressionStats();
    }

    OptimizedAttributeStorage::StorageStats OptimizedAttributeStorage::getStorageStats() const {
        StorageStats stats;
    ...
    void OptimizedAttributeStorage::buildOffsetIndex() {
        if (index_built_) {
            return;
        }
    ...
    const std::unordered_map<uint64_t, int64_t>& OptimizedAttributeStorage::getOffsetIndex() {
        if (!index_built_) {
            buildOffsetIndex();
        }
